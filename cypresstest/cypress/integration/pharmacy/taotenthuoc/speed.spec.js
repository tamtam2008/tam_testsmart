...
import thuocmoi from '../../data/thuoc_moi.json';

// console.log(thuocmoi.data)
...
describe('First step', function(){
  ...
  it('Finds an elements', function(){
    ...
    for(let i = 0; i <= 10; i++){
      ...
      cy.get('input[name="createSM.code1"]').clear().type(thuocmoi.data[i].tenthuoc)
      cy.wait(setting.wait)
      cy.get('input[name="createSM.registerNumber1"]').clear().type(thuocmoi.data[i].sodangki)
      cy.wait(setting.wait)
      cy.get('div[name="infos"]').click({force: true})
      cy.wait(setting.waitdata)
      cy.contains(thuocmoi.data[i].quycach).click({force: true})
      //cy.get('div[name="infos"]').select(thuocmoi.data[i].quycach)
      cy.wait(setting.wait)
